...
class RNNGate(nn.Module):
    ...
    def repackage_hidden(self):
        self.hidden = repackage_hidden(self.hidden).cuda()
    ...
    def forward(self, x):
        ...
        self.rnn = self.rnn.cpu()
        out, self.hidden = self.rnn(x.view(1, batch_size, -1).cpu(), self.hidden)
...
class ResNetRecurrentGateRL(nn.Module):
    ...
    def forward(self, x):
        ...
        # reinitialize hidden units
        self.control.hidden = self.control.init_hidden(batch_size).cuda()
